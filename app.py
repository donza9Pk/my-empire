...
# --- 1. SETUP ---
st.set_page_config(page_title="Donza Wolf Empire | Master Vault", layout="wide")

# Connection: High-speed Ankr RPC for BSC
...
def get_live_price(token_addr):
    try:
        # PancakeSwap API (Best for custom tokens like MetaDoge/Blkwhale)
        url = f"https://pancakeswap.info{token_addr}"
        response = requests.get(url).json()
        return float(response['data']['price'])
    except:
        try:
            # Fallback to CoinGecko for BTCB/WBTC
            url = f"https://coingecko.com{token_addr.lower()}&vs_currencies=usd"
            res = requests.get(url).json()
            return float(res[token_addr.lower()]['usd'])
        except:
            return 0.000000001 

# --- 3. THE COMPLETE DATA (9 WALLETS & 30 TOKENS) ---
WALLETS = {
...
# ALL 30 TOKENS FROM YOUR SECRET LIST
TOKENS = {
    "Blkwhale_1": "0xc0E6AD13BD58413Ed308729b688d601243E1CF77",
    "Opbnb_1": "0xCc9f1EE0eC5E2372A7ce06521B57393De0A94444",
    "Opbnb_2": "0x77Ff107C4539698A8e60d6c280231BEfC58E4444",
    ...
    "WBTC_1": "0xef8C9335C25AF537496ca56f8a2DdF2Dea4f7777",
    "BNB": "0x3413129301cB13Ea9449F6cf295e3Da6ADe84444",
    "Marco": "0x963556de0eb8138E97A85F0A86eE0acD159D210b",
    "WBTC_2": "0xA01b9cAFE2230093fbf0000B43701E03717F77cE",
    "WBTC_3": "0x040f46776aE3800fA92cCb0260f18c69D882674B",
    "Blkwhale_2": "0xc0E6AD13BD58413Ed308729b688d601243E1CF77",
    "Cake_LP": "0x0eD7e52944161450477ee417DE9Cd3a859b14fD0",
    "Eyed": "0xeDd9f422bC4D8E55c93a4E2fE64615f8dAb27223",
    "CPL_WBTC": "0xf5115d0271b97CcFEc86793B3770A69F3494BBf9",
    "Stable": "0x011EBe7d75E2C9D1E0bD0be0bEf5C36f0A90075F",
    "BabyDoge_Eth_1": "0xAC57De9C1A09FeC648E93EB98875B212DB0d460B",
    "Clutch": "0x9F49bEebdF23b4b050defB2e3B1562a5fFc45EF6",
    "WBTC_4": "0xef8C9335C25AF537496ca56f8a2DdF2Dea4f7777",
    "MM72": "0xdF9e1A85dB4f985D5BB5644aD07d9D7EE5673B5E",
    "MetaDoge_BNB_1": "0x8530b66ca3DDf50E0447eae8aD7eA7d5e62762eD",
    "MetaDoge_BNB_2": "0xf3B185ab60128E4C08008Fd90C3F1F01f4B78d50",
    "Wton": "0xB1deEe21b08E23E88A91A18fC0F20E31aeD6fD8c",
    "BabyDoge_Eth_2": "0xAC57De9C1A09FeC648E93EB98875B212DB0d460B",
    "Ton": "0x5926955a02a43235Be1711941D12488F1b7a7777",
    "MetaDoge": "0x8530b66ca3DDf50E0447eae8aD7eA7d5e62762eD",
    "Btcbsc": "0x291BE4B6eA733148ad1D2489dd59dA28F92b710E",
...
def scan_assets(args):
    wn, wa, tn, ta = args
    try:
        contract = w3.eth.contract(address=w3.to_checksum_address(ta), abi=ABI)
        raw = contract.functions.balanceOf(w3.to_checksum_address(wa)).call()
        if raw > 0:
            dec = contract.functions.decimals().call()
            qty = raw / (10**dec)
            price = get_live_price(ta)
            return {"Vault": wn, "Asset": tn, "Qty": qty, "Value": qty * price}
    except: return None

# --- 5. INTERFACE ---
st.title("🐺 The Donza Wolf Empire v5.7")
if st.button("🚀 EXECUTE FULL EMPIRE SYNC"):
    st.cache_data.clear()
    st.rerun()

all_tasks = [(wn, wa, tn, ta) for wn, wa in WALLETS.items() for tn, ta in TOKENS.items()]
with st.status(f"Scanning {len(all_tasks)} Chain Combinations...", expanded=False) as status:
    with ThreadPoolExecutor(max_workers=30) as exe:
        results = list(exe.map(scan_assets, all_tasks))
    final_data = [r for r in results if r is not None]
    status.update(label="Empire Fully Loaded!", state="complete")

total = sum(d['Value'] for d in final_data)
st.metric("TOTAL EMPIRE NET WORTH", f"${total:,.2f}")

if final_data:
    df = pd.DataFrame(final_data)
    st.table(df.style.format({"Qty": "{:,.4f}", "Value": "${:,.2f}"}))
else:
    st.error("⚠️ DATA ALERT: All balances returned $0.00. Check if Wallet Addresses are correct or try later.")
    "Alpha Vault": "0x0eC7f1D93d2A39f695587cA1123482FC3e867e45",
    "Shadow Reserve": "0x70175abF9aCCa0FC10D3057eD876e87a464a1cC6",
    "OKX Fortress": "0x87cA7A6f57435b104BfA28B00596b4da6a64C2E7",
    "Wolf-04": "0xbB35180acA3eD3EA2028928Eb0BEa0EbD3D5c855",
