...
# --- 5. INTERFACE ---
st.title("🐺 The Donza Wolf Empire v5.8")
if st.button("🚀 EXECUTE FULL EMPIRE SYNC"):
    st.cache_data.clear()
    st.rerun()

all_tasks = [(wn, wa, tn, ta) for wn, wa in WALLETS.items() for tn, ta in TOKENS.items()]
with st.status(f"Scanning {len(all_tasks)} Combinations...", expanded=False) as status:
...
if final_data:
    ...
    st.error("⚠️ DATA ALERT: All balances returned $0.00. Please ensure contracts are BEP-20.")
